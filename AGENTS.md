...
| `routes/admin.py`       | 管理后台 `/admin/*`（总览/会话踢下线/管理员管理/推送/缓存/水电费/系统状态/审计日志 + 校历/通知上传 + 公众号文章管理）。鉴权：`Depends(require_admin)`（`session.is_admin`），owner 才能增删管理员。敏感操作写 `admin_audit_log`。`admin_role_of()` 供 `/internal/create-session` 标记 is\_admin。`list_published_admin_notices()` 供 `/academic/notices` 并入「校历」类通知                                                               |
| `wechat_service.py`     | 公众号文章同步（可插拔 `WechatFetcher` 通道）：① `RssFetcher`（wechatrss 第三方 RSS，`WECHAT_RSS_URL` 含私人 token，任何日志/响应必须经 `_mask_url()` 脱敏）② `AlbumFetcher`（微信公开合集接口 appmsgalbum，匿名免凭据，`WECHAT_ALBUM_URLS` 最多支持 10 个，兼容 `WECHAT_ALBUM_URL`）。`default_fetcher()` 按配置优先级 RSS > 多合集选通道，`active_channel()` 供管理后台展示。另含 `fetch_article_meta()`（og 元数据，粘贴链接兜底）、`sync_articles()`/`should_sync()` 惰性同步。表：`WxArticle`/`WechatSyncState` |
